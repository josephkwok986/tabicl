...
class TabICL(nn.Module):
    ...
    def _train_forward(
        ...
        if self.use_activation_checkpointing:
            ...
            def _col(x: Tensor) -> Tensor:
                return self.col_embedder(x, d=d, train_size=None if embed_with_test else train_size)

            col_in = X.contiguous().clone()
            col_out = checkpoint(_col, col_in, use_reentrant=False)
            # 关键：打断潜在的 view 关系，避免下一段对其原地写
            col_out = col_out.contiguous().clone()

            # === 2) Row-wise interaction ===
            def _row(x: Tensor) -> Tensor:
                return self.row_interactor(x, d=d)

            row_in = col_out  # 已 clone
            #representations = checkpoint(_row, row_in, use_reentrant=False)
            representations = _row(row_in)

            # === 3) Dataset-wise in-context learning ===
            def _icl(x: Tensor) -> Tensor:
                return self.icl_predictor(x, y_train=y_train)

            icl_in = representations
            #out = checkpoint(_icl, icl_in, use_reentrant=False)
            out = self.icl_predictor(icl_in, y_train=y_train)
