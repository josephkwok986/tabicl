...
python mycmd/server/infer_server/single_gpu_worker.py --config mycmd/server/infer_server/worker_config.yaml

该脚本基于 ``base_components`` 提供的执行框架，实现一个“单 GPU、永不自停、
人工信号终止”的任务消费进程。主进程负责：

1. 读取配置（默认位于 ``mycmd/server/infer_server/worker_config.yaml``，可通过 ``--config`` 覆盖）
...
def _load_settings() -> ServiceSettings:
    ...
    output_root = Path(worker_cfg.get("output_root", "mycmd/server/infer_server/runtime/results")).expanduser().resolve()
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="配置文件路径（缺省为 mycmd/server/infer_server/worker_config.yaml）",
    )
