...
扫描 `server/sample_data`（可覆盖）目录下的全部 ``.csv`` 数据集，并为每个文件
构造一个队列任务。每个任务的 ``extras`` 字段携带预测所需的路径、格式以及
样本数量提示，供单 GPU worker 直接消费。

示例：

    python server/produce_server/task_producer.py \\
        --input-dir server/sample_data \\
        --queue-dir server/infer_server/runtime/queue \\
...
DEFAULT_CONFIG_PATH = Path("server/produce_server/producer_config.yaml")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="task_system 配置文件路径（默认使用 server/infer_server/worker_config.yaml）",
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=None,
        help="待扫描的输入目录（默认：配置文件或 server/sample_data）",
    )
    parser.add_argument(
        "--queue-dir",
        type=Path,
        default=None,
        help="任务队列所在目录（默认：配置文件或 server/infer_server/runtime/queue）",
    )
    parser.add_argument(
        "--job-id",
        type=str,
...
def main() -> int:
    ...
    if task_cfg_path is None:
        task_cfg_path = Path("server/infer_server/worker_config.yaml")
    ...
    input_dir = args.input_dir or _cfg_path("input_dir", "server/sample_data")
    queue_dir = args.queue_dir or _cfg_path("queue_dir", "server/infer_server/runtime/queue")
